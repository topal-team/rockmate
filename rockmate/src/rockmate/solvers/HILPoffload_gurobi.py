...
class ModelGurobi:
    ...
    def __init__(
        ...
        self.nComp = []  # number to run R, =nOpts if bwd, =nOpts+1 if fwd
        self.time = []
        self.overhead = []
        ...
        for i in range(T):
            for t in range(T):
                self.sumComp[(i, t)] = quicksum(
                    self.Comp[i][t, o] for o in range(self.nComp[i])
                )
        ...
        for j in range(J):
            for t in range(T + 1):
                self.sumAlivePT[(j, t)] = quicksum(
                    self.AlivePT[j][t, o] for o in range(len(self.list_list_sched[j]))
                )
        ...
        self.md.addLConstr(
            quicksum(self.sumComp[(i, t)] for t in range(T) for i in range(t + 1, T)),
            EQ,
            0,
        )
        self.md.addLConstr(
            quicksum(
                self.sumAlivePT[(self.hcn2sub_c[i], t)]
        ...
        self.md.addLConstr(
            quicksum(self.sumComp[(t, t)] for t in range(T)),
            GRB.EQUAL,
            T,
        )
        ...
        for t in range(T):
            for i in range(T):
                ...
                if self.acc_mem:  # Ocp can be counted only after each stage
                    save_mem = quicksum(
