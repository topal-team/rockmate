...
def PartitionedGraph_to_H_graph(
    ...
    for hdn in hg.list_hdn:
        ...
        for kcn in kdn.deps if kdn.mt != constants.init_target_string else kdn.deps_global:
            if kcn is not kg.loss_kcn:
                if kcn in dict_kcn_to_hcn:
                    hcn = dict_kcn_to_hcn[kcn]
                    hdn.deps.add(hcn)
                    hcn.users.add(hdn)
        for kcn in kdn.users_real if kdn.mt != constants.init_target_string else kdn.users_global:
            if kcn is not kg.loss_kcn:
                if kcn in dict_kcn_to_hcn:
                    hcn = dict_kcn_to_hcn[kcn]
